...
export default function SettingsPage() {
  ...
  const handleLogoUpload = (event) => {
    ...
    if (!file) { setLogoName(""); setLogoError(""); return; }
    if (file.size > 2 * 1024 * 1024) {
      setLogoName(""); setLogoError("Ảnh vượt quá 2MB.");
      event.target.value = ""; return;
    }
    setLogoName(file.name); setLogoError("");
  };
      ...
      <div className="admin-page">
        ...
        <div className="grid gap-4 xl:grid-cols-[320px_1fr]">
          ...
          <section className="admin-panel-pad">
            ...
            >
              <span>
                <span className="block text-sm font-bold text-emerald-700">Tự động tính thuế</span>
                <span className="block text-xs font-medium text-emerald-600/80">Áp dụng cho mọi đơn hàng</span>
              </span>
              <span className={`flex h-7 w-12 items-center rounded-full p-1 transition-colors ${autoTax ? "justify-end bg-emerald-600" : "justify-start bg-gray-300"}`}>
          ...
          <section className="admin-panel-pad">
            <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button type="button" className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold text-emerald-600 transition-colors hover:bg-emerald-50">
                <Plus size={16} weight="bold" /> Thêm phương thức
              </button>
            </div>
                ...
                return (
                  <article
                    ...
                    className={`rounded-xl border p-4 transition-all hover:-translate-y-0.5 hover:shadow-md ${
                      method.active ? "border-emerald-100 bg-white shadow-sm" : "border-gray-100 bg-gray-50 opacity-70"
                    } ${method.active ? "ring-2 ring-emerald-100" : ""}`}
                  >
                    <div className="mb-5 flex items-start justify-between gap-3">
                      ...
                      {method.toggleable ? (
                        ...
                        <button
                          type="button"
                          onClick={() => toggleMethod(method.id)}
                          className={`flex h-6 w-6 items-center justify-center rounded-md text-white transition-colors ${method.active ? "bg-emerald-600" : "bg-gray-200"}`}
                          aria-pressed={method.active}
                        >
                          {method.active && <span className="text-xs font-bold">✓</span>}
                        </button>
                      )}
                    </div>
        ...
        <div className="grid gap-4 xl:grid-cols-[390px_1fr]">
          ...
          <section className="rounded-[18px] border border-slate-200/80 bg-slate-50 p-4">
            ...
            <div className="mx-auto max-w-[280px] bg-white px-6 py-6 shadow-[0_22px_50px_rgba(15,23,42,0.14)]">
              ...
              <div className="space-y-2 border-b border-dashed border-gray-200 py-4">
                ...
                <div className="flex justify-between text-xs font-bold text-gray-900">
                  <span>VAT ({autoTax ? safeVatRate : 0}%):</span><span>{formatMoney(vatAmount)}</span>
                </div>
          ...
          <section className="admin-panel-pad">
            ...
            <div className="space-y-4">
              <div>
                <label className="mb-2 block text-xs font-bold text-gray-700">Logo nhà hàng</label>
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                  ...
                  <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-emerald-600 px-6 py-3 text-sm font-bold text-white transition-colors hover:bg-emerald-700">
                    <UploadSimple size={17} weight="bold" />
                    Tải ảnh lên
                    <input type="file" accept="image/png,image/jpeg,image/gif" className="sr-only" onChange={handleLogoUpload} />
                  </label>
                </div>
                <p className={`mt-2 text-xs font-medium ${logoError ? "text-red-500" : "text-gray-400"}`}>
